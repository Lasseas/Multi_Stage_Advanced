...
read_all_sheets('Test_data_advanced.xlsx')
...
model.LoadShiftingWindow = pyo.Set(dimen = 2, ordered = True) #Subset of time periods for load shifting
model.Month = pyo.Set(ordered = True) #Set of months
model.Nodes = pyo.Set(ordered=True) #Set of Nodess
model.NodesInMonth = pyo.Set(within = model.Nodes) #Subset of time periods in month m
...
model.LastStageInLoadShiftingWindow = pyo.Set(dimen = 2, ordered = True)
model.FirstStage = pyo.Set(ordered = True)
model.LastStageInMonth = pyo.Set(dimen = 2, ordered = True)

#Reading the Sets, and loading the data
#data.load(filename="Set_of_TimeSteps.tab", format="set", set=model.Time)
data.load(filename="Subset_LoadShiftWindow.tab", format="set", set=model.LoadShiftingWindow)
data.load(filename="Set_of_Month.tab", format = "set", set=model.Month)
data.load(filename="Subset_of_NodesInMonth.tab", format = "set", set=model.NodesInMonth)
...
data.load(filename="Subset_Plan_Nodes.tab", format = "set", set = model.Nodes_Plan)
data.load(filename="Subset_OPS_Nodes.tab", format = "set", set = model.Nodes_RT)
...
data.load(filename="Subset_LastNodesInShiftWindow.tab", format = "set", set = model.LastStageInLoadShiftingWindow)
data.load(filename="Subset_FirstStage_Nodes.tab", format = "set", set = model.FirstStage)
data.load(filename="Subset_LastNodesInMonth.tab", format = "set", set = model.LastStageInMonth)

"""
PARAMETERS
"""
#Declaring Parameters
model.Cost_Energy = pyo.Param(model.Nodes, model.Technology, default=0.0)  # Cost of using energy source i at time t
model.Cost_Battery = pyo.Param(model.FlexibleLoad)
model.Cost_Export = pyo.Param(model.Nodes, model.Technology)  # Income from exporting energy to the grid at time t
...
model.aFRR_Up_Capacity_Price = pyo.Param(model.Nodes, default=0.0)  # Capacity Price for aFRR up regulation 
model.aFRR_Dwn_Capacity_Price = pyo.Param(model.Nodes, default=0.0)  # Capcaity Price for aFRR down regulation
model.aFRR_Up_Activation_Price = pyo.Param(model.Nodes, default=0.0)  # Activation Price for aFRR up regulation 
model.aFRR_Dwn_Activation_Price = pyo.Param(model.Nodes, default=0.0)  # Activatioin Price for aFRR down regulation 
model.Spot_Price = pyo.Param(model.Nodes, default=0.0)
model.Intraday_Price = pyo.Param(model.Nodes, default=0.0)
...
model.Demand = pyo.Param(model.Nodes, model.EnergyCarrier, default = 0)  # Energy demand 
...
model.Availability_Factor = pyo.Param(model.Nodes, model.Technology) #Availability factor for technology delivering to energy carrier 
...
model.Activation_Factor_UP_Regulation = pyo.Param(model.Nodes, default = 0) # Activation factor determining the duration of up regulation
model.Activation_Factor_DWN_Regulation = pyo.Param(model.Nodes, default = 0) # Activation factor determining the duration of dwn regulation
model.Activation_Factor_ID_Up = pyo.Param(model.Nodes, default = 0) # Activation factor determining the duration of up regulation
model.Activation_Factor_ID_Dwn = pyo.Param(model.Nodes, default = 0) # Activation factor determining the duration of dwn regulation
...
"""
#Declaring Variables
model.x_UP = pyo.Var(model.Nodes, domain= pyo.NonNegativeReals)
model.x_DWN = pyo.Var(model.Nodes, domain= pyo.NonNegativeReals)
...
model.x_DA_Up = pyo.Var(model.Nodes, domain= pyo.NonNegativeReals)
model.x_DA_Dwn = pyo.Var(model.Nodes, domain= pyo.NonNegativeReals)
model.x_ID_Up = pyo.Var(model.Nodes, domain= pyo.NonNegativeReals)
model.x_ID_Dwn = pyo.Var(model.Nodes, domain= pyo.NonNegativeReals)
...
model.y_out = pyo.Var(model.Nodes, model.TechnologyToEnergyCarrier, domain = pyo.NonNegativeReals)
model.y_in = pyo.Var(model.Nodes, model.EnergyCarrierToTechnology, domain = pyo.NonNegativeReals)
model.y_activity = pyo.Var(model.Nodes, model.Technology, model.Mode_of_operation, domain = pyo.NonNegativeReals)
#model.z_export = pyo.Var(model.Nodes, model.Time, model.EnergyCarrier, domain = pyo.NonNegativeReals, bounds = (0, 0))
model.q_charge = pyo.Var(model.Nodes, model.FlexibleLoad, domain= pyo.NonNegativeReals)
model.q_discharge = pyo.Var(model.Nodes, model.FlexibleLoad, domain= pyo.NonNegativeReals)
model.q_SoC = pyo.Var(model.Nodes, model.FlexibleLoad, domain= pyo.NonNegativeReals)
...
def objective(model):
    obj_expr = model.I_inv + model.I_GridTariff + sum(model.Node_Probability[n] * (model.I_cap_bid[n] + model.I_activation[n] + model.I_DA[n] + model.I_ID[n] + model.I_OPEX[n]) for n in model.Nodes_RT) 
...
def cost_capacity_bid(model, n, p):
    return model.I_cap_bid[n] == - (model.aFRR_Up_Capacity_Price[n] * model.x_UP[p] + model.aFRR_Dwn_Capacity_Price[n] * model.x_DWN[p])
model.CapacityBidCost = pyo.Constraint(model.Parent_Node, rule=cost_capacity_bid)

def cost_activation(model, n, p):
    return model.I_activation[n] == (- model.Activation_Factor_UP_Regulation[n] * model.aFRR_Up_Activation_Price[n] * model.x_UP[p]
            + model.Activation_Factor_DWN_Regulation[n] * model.aFRR_Dwn_Activation_Price[n] * model.x_DWN[p])
model.ActivationCost = pyo.Constraint(model.Parent_Node, rule=cost_activation)

def cost_DA(model, n, p):
    return model.I_DA[n] == model.Spot_Price[n] * (model.x_DA_Up[p] - model.x_DA_Dwn[p])
model.DACost = pyo.Constraint(model.Parent_Node, rule=cost_DA) 

def cost_ID(model, n, p):
    return model.I_ID[n] == model.Intraday_Price[n] * (
            model.Activation_Factor_ID_Up[n] * model.x_ID_Up[p] 
            - model.Activation_Factor_ID_Dwn[n] * model.x_ID_Dwn[p]
            )
model.IDCost = pyo.Constraint(model.Parent_Node, rule=cost_ID)    

def cost_opex(model, n):
    return model.I_OPEX[n] == (sum(
                model.y_out[n, i, e, o] * (model.Cost_Energy[n, i] 
                + model.Carbon_Intensity[i, o] * model.Cost_Emission)
                for (i, e, o) in model.TechnologyToEnergyCarrier #MÅ OGSÅ LEGGE TIL FOR Y_IN HVIS DET SKAL VÆRE EXPORT
            ) 

            + sum(model.y_in[n, i, e, o] * model.Cost_Export[n, i] for (i,e,o) in model.EnergyCarrierToTechnology)

            + sum(model.Cost_Battery[b] * model.q_discharge[n, b] for b in model.FlexibleLoad)
    )
model.OPEXCost = pyo.Constraint(model.Nodes_RT, rule=cost_opex)

def Cost_Grid_Tariff(model):
    return model.I_GridTariff == sum(sum(model.Node_Probability[n] * model.Cost_Grid * model.y_max[n, m] for m in model.Month) for (_,n) in model.LastStageInMonth) 
model.CostGridTariff = pyo.Constraint(rule = Cost_Grid_Tariff)
...
def energy_balance(model, n, e):
    return (
        model.d_flex[n, e]
        == sum(sum(model.y_out[n, i, e, o] for i in model.Technology if (i,e,o) in model.TechnologyToEnergyCarrier)
        - sum(model.y_in[n, i, e, o] for i in model.Technology if(i,e,o) in model.EnergyCarrierToTechnology) for o in model.Mode_of_operation)
        - sum(
            model.Charge_Efficiency[b] * model.q_charge[n, b] - model.q_discharge[n, b]
            for b in model.FlexibleLoad if (b,e) in model.FlexibleLoadForEnergyCarrier
        )
    )
model.EnergyBalance = pyo.Constraint(model.Nodes_RT, model.EnergyCarrier, rule=energy_balance)

#####################################################################################
########################### MARKET BALANCE DA/ID/RT #################################
#####################################################################################
 
def market_balance_import_plan(model, n, i, e, o):
    if (i, e, o) == ("Power_Grid", "Electricity", 1):
        return (model.y_out[n, i, e, o] == model.x_DA_Up[n] + model.x_ID_Up[n] + model.x_DWN[n])
    else:
        return pyo.Constraint.Skip      
model.MarketBalanceImportPlan = pyo.Constraint(model.Nodes_Plan, model.TechnologyToEnergyCarrier, rule = market_balance_import_plan)

def market_balance_export_plan(model, n, i, e, o):
    if (i, e, o) == ("Power_Grid", "Electricity", 2):
        return (model.y_in[n, i, e, o] == model.x_DA_Dwn[n] + model.x_ID_Dwn[n] + model.x_UP[n])
    else:
        return pyo.Constraint.Skip      
model.MarketBalanceExportPlan = pyo.Constraint(model.Nodes_Plan, model.EnergyCarrierToTechnology, rule = market_balance_export_plan)

def market_balance_import_RT(model, n, p, i, e, o):
    if (i, e, o) == ("Power_Grid", "Electricity", 1) and n in model.Nodes_RT:
        return (model.y_out[n, i, e, o] == model.x_DA_Up[p] + model.Activation_Factor_ID_Up[n]*model.x_ID_Up[p] + model.Activation_Factor_DWN_Regulation[n] * model.x_DWN[p])
    else:
        return pyo.Constraint.Skip      
model.MarketBalanceImportRT = pyo.Constraint(model.Parent_Node, model.TechnologyToEnergyCarrier, rule = market_balance_import_RT)

def market_balance_export_RT(model, n, p, i, e, o):
    if (i, e, o) == ("Power_Grid", "Electricity", 2) and n in model.Nodes_RT:
        return (model.y_in[n, i, e, o] == model.x_DA_Dwn[p] + model.Activation_Factor_ID_Dwn[n]*model.x_ID_Dwn[p] + model.Activation_Factor_UP_Regulation[n] * model.x_UP[p])
    else:
        return pyo.Constraint.Skip      
model.MarketBalanceExportRT = pyo.Constraint(model.Parent_Node, model.EnergyCarrierToTechnology, rule = market_balance_export_RT)


def Max_ID_Adjustment(model, n):
        return (model.x_ID_Up[n] + model.x_ID_Dwn[n] <= 10)
model.MaxIDAdjustment = pyo.Constraint(model.Nodes, rule = Max_ID_Adjustment)
...
def conversion_balance_out(model, n, i, e, o):
        return (model.y_out[n, i, e, o] == model.y_activity[n, i, o] * model.Technology_To_EnergyCarrier_Efficiency[i, e, o])     
model.ConversionBalanceOut = pyo.Constraint(model.Nodes_RT, model.TechnologyToEnergyCarrier, rule = conversion_balance_out)

def conversion_balance_in(model, n, i, e, o):
        return (model.y_in[n, i, e, o] == model.y_activity[n, i, o] * model.EnergyCarrier_To_Technlogy_Efficiency[i, e, o])           
model.ConversionBalanceIn = pyo.Constraint(model.Nodes_RT, model.EnergyCarrierToTechnology, rule = conversion_balance_in)

#####################################################################################
########################### TECHNOLOGY RAMPING CONSTRAINTS ##########################
#####################################################################################

def Ramping_Technology(model, n, p, i, e, o):
        if n not in model.FirstStage:
            return (model.y_out[n, i, e, o] - model.y_out[p, i, e, o] <= model.Ramping_Factor[i] * (model.Initial_Installed_Capacity[i] + model.v_new_tech[i]))
        else:
            return (model.y_out[n, i, e, o] <= model.Ramping_Factor[i] * (model.Initial_Installed_Capacity[i] + model.v_new_tech[i]))        
model.RampingTechnology = pyo.Constraint(model.Parent_Node, model.TechnologyToEnergyCarrier, rule = Ramping_Technology)

#####################################################################################
############## HEAT PUMP LIMITATION - MÅ ENDRES I HENHOLD TIL INPUTDATA #############
#####################################################################################

def heat_pump_input_limitation_LT(model, n):
    return (
        model.y_out[n, 'HeatPump_LT', 'LT', 1] - model.y_in[n, 'HeatPump_LT', 'Electricity', 1]
        <= model.Available_Excess_Heat * (model.Demand[n, 'LT'])# + model.Demand[s, t, 'HT'])
    )
model.HeatPumpInputLimitationLT = pyo.Constraint(model.Nodes_RT, rule=heat_pump_input_limitation_LT)

def heat_pump_input_limitation_MT(model, n):
    return (
        model.y_out[n, 'HeatPump_MT', 'MT', 1] - model.y_in[n, 'HeatPump_MT', 'Electricity', 1]
        <= model.Available_Excess_Heat * (model.Demand[n, 'MT'])# + model.Demand[s, t, 'HT'])
    )
model.HeatPumpInputLimitationMT = pyo.Constraint(model.Nodes_RT, rule=heat_pump_input_limitation_MT)
...
def load_shifting_balance(model, n, e):
    if any(n == stage[1] for stage in model.LastStageInLoadShiftingWindow):
        ...
        return pyo.Constraint.Skip
model.LoadShiftingBalance = pyo.Constraint(model.Nodes, model.EnergyCarrier, rule = load_shifting_balance)

def no_up_shift_outside_window(model, n, e):
...
model.DefiningFlexibleDemand = pyo.Constraint(model.Nodes_RT, model.EnergyCarrier, rule = Defining_flexible_demand)

###########################################################
############## MAX ALLOWABLE UP/DOWN SHIFT ################
###########################################################

def Max_total_up_dwn_load_shift(model, n, e):
        return model.Up_Shift[n, e] + model.Dwn_Shift[n, e] <= model.Up_Shift_Max * model.Demand[n,e]        
model.MaxTotalUpDwnLoadShift = pyo.Constraint(model.Nodes_RT, model.EnergyCarrier, rule=Max_total_up_dwn_load_shift)
...
def flexible_asset_charge_discharge_limit(model, n, b, e):
        return (
            model.q_charge[n, b] 
            + model.q_discharge[n, b] / model.Discharge_Efficiency[b] 
            <= model.Max_charge_discharge_rate[b] + model.Energy2Power_Ratio[b] * model.v_new_bat[b]
        )
model.FlexibleAssetChargeDischargeLimit = pyo.Constraint(model.Nodes_RT, model.FlexibleLoadForEnergyCarrier, rule=flexible_asset_charge_discharge_limit)

def state_of_charge(model, n, p, b, e):
    if n in model.FirstStage:  # Initialisation of flexible assets
        return (
            model.q_SoC[n, b]
        ...
        return (
            model.q_SoC[n, b]
            == model.q_SoC[p, b] * (1 - model.Self_Discharge[b])
            + model.q_charge[n, b]
            - model.q_discharge[n, b] / model.Discharge_Efficiency[b]
        )
model.StateOfCharge = pyo.Constraint(model.Parent_Node, model.FlexibleLoadForEnergyCarrier, rule=state_of_charge)

def end_of_horizon_SoC(model, n, b, e):
    if n == model.LastStageInMonth:
        return model.q_SoC[n, b] == model.Initial_SOC[b] * (model.Max_Storage_Capacity[b] + model.v_new_bat[b])
    else:
        return pyo.Constraint.Skip
model.EndOfHorizonSoC = pyo.Constraint(model.Nodes_RT, model.FlexibleLoadForEnergyCarrier, rule = end_of_horizon_SoC)


def flexible_asset_energy_limit(model, n, b, e):
    return model.q_SoC[n, b] <= model.Max_Storage_Capacity[b] + model.v_new_bat[b]
model.FlexibleAssetEnergyLimits = pyo.Constraint(model.Nodes_RT, model.FlexibleLoadForEnergyCarrier, rule=flexible_asset_energy_limit)

####################################################
############## AVAILABILITY CONSTRAINT #############
####################################################

def supply_limitation(model, n, i, e, o):
    return (sum(model.y_out[n, i, e, o] for e,o in model.EnergyCarrier * model.Mode_of_operation if (i,e,o) in model.TechnologyToEnergyCarrier)  
            <= model.Availability_Factor[n, i] * (model.Initial_Installed_Capacity[i] + model.v_new_tech[i]))
model.SupplyLimitation = pyo.Constraint(model.Nodes_RT, model.TechnologyToEnergyCarrier, rule=supply_limitation)

##############################################################
############## EXPORT LIMITATION AND GRID TARIFF #############
##############################################################

def export_limitation(model, n, i, e, o):
    if (i, e, o) == ('Power_grid', 'Electricity', 2):
        return model.y_in[n, i, e, o] <= model.Max_Export
    else:
        return pyo.Constraint.Skip
model.ExportLimitation = pyo.Constraint(model.Nodes_RT, model.EnergyCarrierToTechnology, rule=export_limitation)

def peak_load(model, n, i, e, o, m):
    if i == 'Power_Grid' and e == 'Electricity':
        return (model.y_out[n, i, e, o] <= model.y_max[n, m])
    else:
        return pyo.Constraint.Skip
model.PeakLoad = pyo.Constraint(model.Nodes_RT, model.TechnologyToEnergyCarrier, model.Month, rule=peak_load)

def Node_greater_than_parent(model, n, p, m):
    return model.y_max[p, m] <= model.y_max[n, m]
model.NodeGreaterThanParent = pyo.Constraint(model.Parent_Node, model.Month, rule = Node_greater_than_parent)
...
def Carbon_Emission_Limit(model, n):
    total_emission = sum(
        model.y_activity[n, i, o] * model.Carbon_Intensity[i, o]
